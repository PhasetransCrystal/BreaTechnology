...
public abstract class BTRegistry<K, V> implements Iterable<V> {
    ...
    public BTRegistry(ResourceLocation registryName) {
    ...
    public void freeze() {
        ...
        if (!checkActiveModContainerIsMMi()) {
            return;
        }
    ...
    private boolean checkActiveModContainerIsMMi() {
        ModContainer container = ModLoadingContext.get().getActiveContainer();
        return container != null && (container.getModId().equals(this.registryName.getNamespace()) ||
                container.getModId().equals(BreaTechnology.MOD_ID) ||
    ...
    public <T extends V> T replace(K key, T value) {
        ...
        if (!containKey(key)) {
            BreaTechnology.LOGGER.warn("[replace] couldn't find key %s in registry %s".formatted(registryName, key));
        }
        registry.put(key, value);
        return value;
    }
    ...
    public static class String<V> extends BTRegistry<java.lang.String, V> {
    ...
    public static class RL<V> extends BTRegistry<ResourceLocation, V> {
        ...
        public V readBuf(FriendlyByteBuf buf) {
            if (buf.readBoolean()) {
                return get(ResourceLocation.bySeparator(buf.readUtf(), ':'));
            }
            return null;
        }
        ...
        @Override
        public V loadFromNBT(Tag tag) {
            return get(ResourceLocation.bySeparator(tag.getAsString(), ':'));
        }
